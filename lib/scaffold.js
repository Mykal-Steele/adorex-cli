...
export async function runInstall(projectPath) {
  ...
  const result = await runCommand(NPM, args, { cwd: projectPath, retries: 1 });

  if (!result.ok) {
    const output =
      result.stderr || result.stdout || result.error?.message || '';
    throw new Error(
      `Failed to install dependencies\nCommand: ${NPM} ${args.join(' ')}${output ? `\n${output}` : ''}`,
    );
  }
...
export async function runPrismaGenerate(projectPath) {
  ...
  const npmArgs = ['exec', '--silent', 'prisma', 'generate'];
  const npmResult = await runCommand(NPM, npmArgs, { cwd: projectPath });
