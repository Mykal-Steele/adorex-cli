...
export async function copyTemplate(templateDir, projectPath) {
	await fs.copy(templateDir, projectPath, { overwrite: false, errorOnExist: true });
}
...
export async function renderTemplateFiles(projectPath, context) {
	...
	const [packageTemplate, envTemplate, schemaTemplate, readmeTemplate] = await Promise.all([
...
export async function runInstall(projectPath) {
	...
	if (!result.ok) {
		...
		throw new Error(
			`Failed to install dependencies\nCommand: ${commandToString(command, args)}${result.error?.message ? `\nCause: ${result.error.message}` : ""}`,
		);
	}
...
export async function runPrismaGenerate(projectPath) {
	...
	const npmExecResult = await runCommand(npmExecCommand, npmExecArgs, { cwd: projectPath });
	...
	spinner.text = "npm exec failed, retrying with npx prisma generate...";
	...
	if (!npxResult.ok) {
		...
		throw new Error(
			`Failed to generate Prisma client. Run manually: npx prisma generate\nCommands tried: ${commandToString(npmExecCommand, npmExecArgs)} | ${commandToString(npxCommand, npxArgs)}${npxResult.error?.message ? `\nCause: ${npxResult.error.message}` : ""}`,
		);
	}
